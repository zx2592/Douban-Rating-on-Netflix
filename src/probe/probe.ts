...
async function traceOne(): Promise<void> {
  ...
  try {
    for (const term of buildSearchTerms(title)) {
      ...
      const candidates = await traceClient.search(term);
      ...
      lines.push('');
      // 第一个检索词有过线候选就够了，和线上逻辑一致。
      if (candidates.some((candidate) => scoreCandidate(query, candidate).raw >= threshold)) break;
    }
    lines.push('—— 排查完成。若所有候选都 ❌，把这份结果整个复制回去。');
  } catch (error) {
