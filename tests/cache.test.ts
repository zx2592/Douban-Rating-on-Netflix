...
describe('RatingCache 读写', () => {
  it('写入后能读出来', async () => {
    ...
    cache.set('r3:a', rating);
    expect((await cache.get('r3:a'))?.rating).toEqual(rating);
  });

  it('未命中结果也会被缓存，避免反复打豆瓣', async () => {
    const storage = memoryStorage();
    const cache = new RatingCache(storage);
    cache.set('r3:none', null);
    const entry = await cache.get('r3:none');
  ...
  it('没写过的键返回 undefined', async () => {
    ...
    expect(await cache.get('r3:missing')).toBeUndefined();
  });
  ...
  it('service worker 重启后（内存丢失）仍能从存储里读回', async () => {
    ...
    first.set('r3:a', rating);
    await first.flush();

    const restarted = new RatingCache(storage);
    expect((await restarted.get('r3:a'))?.rating).toEqual(rating);
  });
});
...
describe('RatingCache 过期', () => {
  it('命中结果在 TTL 内有效、超出后失效', async () => {
    ...
    time += FOUND_TTL_MS - 1;
    expect(await cache.get('r3:a')).toBeDefined();

    time += 2;
    expect(await cache.get('r3:a')).toBeUndefined();
  });
  ...
  it('未命中结果的 TTL 明显更短', async () => {
    ...
    expect(await cache.get('r3:none')).toBeUndefined();
    // 同样的时间跨度下，命中结果应该还活着。
  ...
  it('过期条目在读取时被顺手删掉', async () => {
    ...
    time += FOUND_TTL_MS + 1;
    await cache.get('r3:a');
    expect(storage.data.has('r3:a')).toBe(false);
  });
});
...
describe('RatingCache 清理', () => {
  ...
  it('超出容量上限时丢掉最老的一批', async () => {
    ...
    for (let i = 0; i < 4001; i += 1) {
      storage.data.set(`r3:${i}`, { at: time - 5000 + i, rating });
    }
    cache.set('r3:new', rating);
    await cache.flush();

    const remaining = await cache.size();
    expect(remaining).toBeLessThan(4002);
    // 最老的那条应该首先被淘汰，最新写入的应该还在。
    expect(storage.data.has('r3:0')).toBe(false);
    expect(storage.data.has('r3:new')).toBe(true);
  });

  it('存储里的脏数据不会让读取崩掉', async () => {
    const storage = memoryStorage();
    storage.data.set('r3:bad', 'not an entry');
    const cache = new RatingCache(storage);
    expect(await cache.get('r3:bad')).toBeUndefined();
  });
});
...
describe('sweepLegacyEntries', () => {
  it('清掉旧版本前缀的键，保留新版本和无关数据', async () => {
    const storage = memoryStorage();
    storage.data.set('r:第一代', { at: 1, rating: null });
    // r2 这一代被「搜索访问太频繁 → 未收录」污染过，必须一并清掉。
    storage.data.set('r2:被污染的未收录', { at: 1, rating: null });
    storage.data.set('r3:新条目', { at: 1, rating });
    storage.data.set('settings', { enabled: true });

    expect(await sweepLegacyEntries(storage)).toBe(2);
    expect(storage.data.has('r:第一代')).toBe(false);
    expect(storage.data.has('r2:被污染的未收录')).toBe(false);
    expect(storage.data.has('r3:新条目')).toBe(true);
    expect(storage.data.has('settings')).toBe(true);
  });

  it('没有旧键时不动存储', async () => {
    const storage = memoryStorage();
    storage.data.set('r3:a', { at: 1, rating });
